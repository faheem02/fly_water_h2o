...
function rebuildSupplierLedger($supplier_id) {
    ...
    $purchases = mysqli_query($conn, "SELECT id, purchase_date, credit_amount, invoice_no, voucher_no FROM raw_material_purchases WHERE supplier_id=$supplier_id AND credit_amount > 0");
    while($p = mysqli_fetch_assoc($purchases)) {
        ...
        $rows[] = [
            ...
            'debit' => 0,
            'credit'=> floatval($p['credit_amount']),
            'ref_id' => intval($p['id']),
            'ref_type' => 'purchase'
        ];
    }
...
function validateAmount() {
    ...
    if(hasSupplier && amount > 0) {
        if(currentBalance > 0 && amount > currentBalance) {
            amountHint.innerHTML = '<i class="fas fa-exclamation-triangle text-warning"></i> Exceeds credit balance; this will create an Advance (Rs ' + fmt(amount - currentBalance) + ').';
